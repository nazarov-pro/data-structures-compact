...
public abstract class AbstractSecuredHandler implements HttpHandler {
    ...
    protected AbstractSecuredHandler() {
        switch (serverProperty.getAuth()) {
            ...
            case ServerConstants.PROPERTY_DEFAULT_API_KEY:
    ...
    protected final void printForbidden(HttpExchange httpExchange) throws IOException {
        byte[] bytes = String.format(FORBIDDEN_TEXT, httpExchange.getRequestURI().toString()).getBytes();
        httpExchange.sendResponseHeaders(RESPONSE_CODE_FORBIDDEN, bytes.length);
        OutputStream os = httpExchange.getResponseBody();
        os.write(bytes);
        os.close();
    }
